...
def _convert_sigmoid(node, graph, err):
    ...
    # l_top_relu1 = L.ReLU(l_bottom, name=name, in_place=True)
    # print("AAAAAAAAAAAAA : ", graph.channel_dims, input_name, output_name)
    # graph.channel_dims[output_name] = graph.channel_dims[input_name]
...
def _convert_upsample(node,graph,err):
    ...
    # https://github.com/jnulzl/caffe_plus 里面的upsample 是用的nearest插值
        ...
        # scales = node.input_tensors.get(node.inputs[1]) # opest-10
        ...
        sizes = node.input_tensors.get(node.inputs[3]) # opest-11 sizes N x C x H x W
        height = sizes[2]
        width = sizes[3]
        layer = myf("Upsample", node_name, [input_name], [output_name],
                    upsample_param=dict(
...
def _convert_resize(node,graph,err):
    ...
    # https://github.com/jnulzl/caffe_plus 里面的upsample 是用的nearest插值
        ...
        # scales = node.input_tensors.get(node.inputs[1]) # opset-10 
        # height_scale = scales[2]
        # width_scale = scales[3]
        # layer = myf("Upsample", node_name, [input_name], [output_name],
        #             upsample_param=dict(
        ...
        elif 3 == len(node.inputs):
            scales = node.input_tensors.get(node.inputs[2]) # opest-11 scale N x C x H x W
            layer = myf("Upsample", node_name, [input_name], [output_name],
                        upsample_param=dict(
                            mode = int(0),
                            height_scale = int(scales[2]),
